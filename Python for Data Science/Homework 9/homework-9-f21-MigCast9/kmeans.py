...
def kmeans(point_data, cluster_data):
    ...
    for cluster in clusterList:
        for point in pointList:
            #Here in the line below we get the cluster that is currently closest to this point
            closestCluster = point.closest(clusterList)
            closestCluster.updateCenter()
            while point.moveToCluster(closestCluster) == True:
                point.moveToCluster(closestCluster)
                closestCluster.updateCenter()
            if closestCluster not in clusters:
                clusters.append(closestCluster)
    # for point in pointList:
    #     # for cluster in clusterList:
    #     closestCluster = point.closest(clusterList)
    #     while point.moveToCluster(closestCluster) == True:
    ...
    # for point in pointList:
    #     for cluster in clusterList:
    #         while point.moveToCluster(cluster) == True:
    #              closestPoint = point.closest()
