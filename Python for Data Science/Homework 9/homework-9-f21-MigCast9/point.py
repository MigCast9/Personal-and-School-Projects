...
class Point:
    ...
    def distFrom(self, other):
        ...
        distance = m.sqrt(sum([(self.coords[coordinate] - other.coords[coordinate]) ** 2 for coordinate in range(len(other.coords))]))
...
def makePointList(data):
    ...
    """
    # fill in
    listOutput = [Point(row) for row in data]
    return np.array(listOutput)
